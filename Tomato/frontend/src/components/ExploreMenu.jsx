  ...
  return (
    <div className="mt-8 flex flex-col gap-[20px] mb-4" id="explore-menu">
      ...
      <p className="max-w-[60%] mx-auto text-lg md:text-xl text-center">
        ...
        that cater to every taste and preference. Enjoy the finest flavors
        crafted just for you!
      </p>

      {/* Menu List */}
      <div
        className="menu-list flex overflow-x-auto scrollbar-none gap-[20px] px-4 py-6"
        id="ExploreMenu"
      >
        {menu_list.map((item, index) => (
          <div
            onClick={() => {
              setCategory(item.menu_name === "All" ? "All" : item.menu_name);
            }}
            key={item.id || index}
            className="menu-item flex flex-col items-center bg-gray-50 shadow-lg rounded-lg p-4 transition-transform transform hover:scale-105 min-w-[200px]"
          >
            {/* Menu Image */}
            <img
              src={item.menu_image}
            ...
            />
            {/* Menu Name */}
            <h2 className="menu-item-title text-xl font-semibold text-[#333]">
              {item.menu_name}
            </h2>
          </div>
        ))}
