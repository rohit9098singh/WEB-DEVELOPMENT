...
const Result = () => {
  ...
  const submitHandler = async (e) => {
    ...
    setLoading(true);
    if (input) {
      const generatedImage = await generateImage(input); // Make sure the correct image is returned
      if (generatedImage) {
    ...
    <motion.form
      ...
      {isImageLoaded && (
        <div className="flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full">
          <p onClick={() => { setIsImageLoaded(false); setInput(''); }} className="bg-transparent border border-zinc-900 text-black px-8 py-3 rounded-full cursor-pointer">Generate Other</p>
