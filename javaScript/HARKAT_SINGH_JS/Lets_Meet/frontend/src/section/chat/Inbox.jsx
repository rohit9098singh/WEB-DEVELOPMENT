...
const Inbox = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (profileRef.current && !profileRef.current.contains(event.target) && !buttonRef.current.contains(event.target)) {
        setIsProfileOpen(false);
      }

      if (gifSearchRef.current && !gifSearchRef.current.contains(event.target) && !buttonRef.current.contains(event.target)) {
    ...
    <>
      <div className={`flex h-full w-[95%] flex-col dark:bg-boxdark ${isProfileOpen ? "w-1/2" : "lg:w-3/4"}`}>
        ...
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-stroke dark:bg-boxdark dark:border-strokedark px-5 py-4">
          <div onClick={handleToggleUserInfo} className="flex items-center gap-3 cursor-pointer">
            <div className="h-12 w-12 rounded-full overflow-hidden">
              <img src={User01} alt={"Avatar"} className="h-full w-full object-cover" />
            </div>
            <div className="flex flex-col gap-y-0">
              <h1 className="text-sm font-semibold text-stone-700 dark:text-white truncate">Henry Lohil</h1>
        ...
        <div className="max-h-full overflow-auto px-9 py-7.5 no-scrollbar space-y-3.5 grow">
          ...
          <div className="max-w-fit">
            ...
            <div className="bg-stone-200 rounded-2xl rounded-tl-none px-5 py-3 dark:bg-boxdark-2 inline-block">
              <p>Hey, I want to meet you to inform you of something very important.</p>
            </div>
          ...
          <div className="flex items-center justify-between space-x-4.5 pr-20 mb-3">
            <div className="relative w-full flex justify-between items-center">
              ...
              <div className="absolute right-5 top-1/2 -translate-y-1/2 flex items-center justify-end space-x-4">
               <button onClick={handleMicClick}  className="hover:text-blue-600">
                  <Microphone size={24}/> 
               </button>
            ...
            <button className="flex items-center justify-center bg-blue-500 text-white rounded-md h-9 max-w-9 w-full hover:bg-blue-600" aria-label="Send message">
              <PaperPlaneTilt size={24} />
            </button>
          </div>

          {/** THE GIPHY COMPONENT RENDERS HERE BELOW THE INPUT TAG */}
          {isGifSearchOpen && <div ref={gifSearchRef}> <Giphy /></div>}
        </div>
      </div>
